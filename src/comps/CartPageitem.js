...
function CartPageItem(props) {
  ...
  function addUnits() {
    ...
    if (props.units + newunits < 10) props.updatedadd(props.price);
  }
  function subUnits() {
    props.units + newunits > 1 ? setUnits((prev) => prev - 1) : (temp = 1);
    if (props.units + newunits > 1) props.updatedsub(props.price);
  }
    ...
    <tr>
      ...
      <td>
        <div className="itemnum">
          <div className="subnum" onClick={() => subUnits()}>
            -
          </div>
          <div className="num">{props.units + newunits}</div>
          <div className="addnum" onClick={() => addUnits()}>
            +
          </div>
        </div>
      </td>
