      ...
      if (pattern.test(prod.name.toLowerCase()))
        return (
          <DashTableRow
            ...
            qty={prod.qty}
            qty_purch={prod.purchased_qty}
            status={prod.purchased_status}
      ...
      <div className="myproductsgrid">
        <div className="dashbox dashlarge dashtable">
          <div className="headeropts">
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) => setFindname(e.target.value.toLowerCase())}
              />
            </label>
          ...
          <table>
            <thead>
              <tr>
                <th style={{ color: sort[0] === 0 ? "var(--color)" : "" }}>
                  <h6
                    onClick={() =>
                      sort[1] === 0 ? setSort([0, 1]) : setSort([0, 0])
                    }
                  >
                    No.<i className="fas fa-sort"></i>
                  </h6>
                </th>
                <th style={{ color: sort[0] === 1 ? "var(--color)" : "" }}>
                  <h6
                    onClick={() =>
                      sort[1] === 0 ? setSort([1, 1]) : setSort([1, 0])
                    }
                  >
                    Product Name<i className="fas fa-sort"></i>
                  </h6>
                </th>
                <th style={{ color: sort[0] === 2 ? "var(--color)" : "" }}>
                  <h6
                    onClick={() =>
                      sort[1] === 0 ? setSort([2, 1]) : setSort([2, 0])
                    }
                  >
                    Unit Price<i className="fas fa-sort"></i>
                  </h6>
                </th>
                <th style={{ color: sort[0] === 3 ? "var(--color)" : "" }}>
                  <h6
                    onClick={() =>
                      sort[1] === 0 ? setSort([3, 1]) : setSort([3, 0])
                    }
                  >
                    Qty<i className="fas fa-sort"></i>
                  </h6>
                </th>
                <th style={{ color: sort[0] === 4 ? "var(--color)" : "" }}>
                  <h6
                    onClick={() =>
                      sort[1] === 0 ? setSort([4, 1]) : setSort([4, 0])
                    }
                  >
                    Quantity Sold<i className="fas fa-sort"></i>
                  </h6>
                </th>
                <th style={{ color: sort[0] === 5 ? "var(--color)" : "" }}>
                  <h6
                    onClick={() =>
                      sort[1] === 0 ? setSort([5, 1]) : setSort([5, 0])
                    }
                  >
                    Earnings<i className="fas fa-sort"></i>
                  </h6>
                </th>
                <th style={{ color: sort[0] === 6 ? "var(--color)" : "" }}>
                  <h6
                    onClick={() =>
                      sort[1] === 0 ? setSort([6, 1]) : setSort([6, 0])
                    }
                  >
                    Stock Status<i className="fas fa-sort"></i>
                  </h6>
                </th>
              </tr>
            </thead>
        ...
        <div className="myproductsoptions dashbox" data-updated={updated}>
          ...
          <div
            className="innerprodopts"
            style={{ display: prodsel ? "block" : "none" }}
          >
            <label>
              <h6>Product Name</h6>
              <input
                className="prodnameinp"
                value={name}
            ...
            <label className="imguploadlabel">
              <h6>Product Image</h6>
              <div
                className="uploadercont"
                style={{ backgroundImage: "url(" + img + ")" }}
              >
                <input className="uploadpic" type="file" onChange={uploadImg} />
              </div>
            </label>
            <label>
              <h6>Price (CAD)</h6>
              <div className="currencydiv">$</div>
              <input
                className="priceinp"
            ...
            <button className="saveproduct" onClick={() => editProduct(id)}>
              Save Product
            </button>
            <button className="removeproduct" onClick={() => removeProduct(id)}>
              Remove Product
            </button>
          </div>
          <div
            className="defaultinnerprod"
            style={{ display: prodsel ? "none" : "block" }}
          >
            <h6>
              Select a product to manage<i className="fas fa-th"></i>
            </h6>
          </div>
        </div>
      </div>
