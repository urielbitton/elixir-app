...
function Shop(props) { 
  ...
  const allprods = products.map(prod => {
      if(((prod.price >= pricefilt[0] && prod.price < pricefilt[1])|| pricefilt === "all") && (prod.cat.includes(catfilter) || catfilter === "all") && (prod.color.includes(colorfilt.toLowerCase()) || colorfilt === "all") && (prod.sizes.includes(sizesfilt) || sizesfilt === "all")) {
        return <Item prod={prod} id={prod.id} name={prod.name} img={prod.img} price={prod.price} descript={prod.descript} hot={prod.hot} sale={prod.sale} color={prod.color} cat={prod.cat} sizes={prod.sizes} addcart={prod.addcart} instock={prod.instock} units={prod.units} wishlist={prod.wishlist} qty={prod.qty} compared={prod.compared} ratings={prod.ratings} review={prod.reviews} avgrating={prod.avgrating} updatecartnum={props.updatecartnum} updatesub={props.updatesub} updatewish={props.updatewish} openproduct={props.openproduct} key={prod.id} addwishnum={props.addwishnum} resetunits={props.resetunits} updatecompare={props.updatecompare} updatecompstatus={props.updatecompstatus}/>
      }   
  }) 
